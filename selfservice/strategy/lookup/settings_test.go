...
func createIdentity(t *testing.T, reg driver.Registry) (*identity.Identity, []identity.LookupSecret) {
	codes := make([]identity.LookupSecret, 12)
	for k := range codes {
		var usedAt sqlxx.NullTime
		if k%3 == 1 {
			usedAt = sqlxx.NullTime(time.Unix(int64(1629199958+k), 0))
		}
		codes[k] = identity.LookupSecret{Code: fmt.Sprintf("key-%d", k), UsedAt: usedAt}
	}
	...
	rc, err := json.Marshal(&identity.CredentialsLookupConfig{LookupSecrets: codes})
	require.NoError(t, err)
